...
export-env {
    ...
    $env.NU_LOG_FILE = $env.NU_LOG_FILE? | default false
    $env.NU_LOG_FILE_NAME = $env.NU_LOG_FILE_NAME? | default (date now | format date "output-%Y%m%dT%H%M%S.log")
	# Add a newline so all text is not on one line.
	# Do not add it to the default in case the user wants another newline format. i.e. \r\n
    $env.NU_LOG_FILE_FORMAT = $env.NU_LOG_FILE_FORMAT? | default "%DATE%|%LEVEL%|%MSG%\n"
}
...
export def custom [
	...
	if ($env.NU_LOG_FILE) {
		...
		}) | save --append $env.NU_LOG_FILE_NAME
	} else {
		print --stderr ([
			["%MSG%" $message]
			["%DATE%" (now)]
